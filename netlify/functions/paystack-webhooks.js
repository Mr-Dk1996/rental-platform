...
const getLedgerProof = async (paymentId, supabaseUrl, serviceRoleKey) => {
    const response = await fetch(
        `${supabaseUrl}/rest/v1/payment_ledger` +
    ...
    if (!response.ok) {
        throw new Error(rows.message || "Unable to retrieve the ledger proof.");
    }
...
exports.handler = async (event) => {
    ...
    try {
        ...
        const ledgerProof = await getLedgerProof(
            finalPayment.id,
            SUPABASE_URL,
            SUPABASE_SERVICE_ROLE_KEY
        );
        ...
        if (!ledgerProof?.ledger_reference) {
            throw new Error("Blockchain V2 ledger reference is missing.");
        }
